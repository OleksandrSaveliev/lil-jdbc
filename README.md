...
3. Run docker image
   `docker run --rm --name lil-postgres -e POSTGRES_PASSWORD=password -d -v $HOME/srv/postgres:/var/lib/postgresql/data -p 5432:5432 postgres:16`

   ## Updated commnad to avoid TimeZone db issue

   `docker run --rm --name lil-postgres -e POSTGRES_PASSWORD=password -e POSTGRES_DB=hplussport --env TZ=Europe/Kyiv --env PGTZ=Europe/Kyiv -d -v $HOME/srv/postgres:/var/lib/postgresql/data -p 5432:5432 postgres`
